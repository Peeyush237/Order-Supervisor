...
async def generate_final_output(payload: dict[str, Any]) -> dict[str, Any]:
    """End-of-run output: LLM-authored summary, important actions, key learnings,
    and recommendations. Falls back to a count-based summary if the LLM output
    can't be parsed."""
    ...
    llm_input = {
        ...
        "events": [e["payload"] for e in event_entries],
        "actions": [a["payload"] for a in action_entries],
    }
    system, user = prompts.build_final_output_messages(llm_input)
    raw = await complete_json(system, user, {"final_output": True})
    ...
    try:
        ...
        final = {
            ...
            "important_actions": list(data.get("important_actions", [])),
        ...
        final = {
            ...
            "important_actions": [
                f"{a['payload'].get('action')} {a['payload'].get('args', {})}"
